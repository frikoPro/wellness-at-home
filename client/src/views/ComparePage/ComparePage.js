...
const ComparePage = () => {
	...
	useEffect(() => {
		const mapped = jacuzzis.map((obj, key) => ({
			...
			averageRating: obj.averageRating ? obj.averageRating : 0,
		}));
	...
	return (
		<Container
			...
			<Row>
				...
				<Col sm={6}>
					<Form.Control
						as="select"
						value={jacuzzi2}
						onChange={(e) => setJacuzzi2(e.target.value)}>
						{jacuzzis.map((jac, index) => (
							<option value={index}>{jac.name}</option>
						))}
					</Form.Control>
			...
			<Row className="justify-content-center">
				<CompareList
					selectedJacuzzi={jacuzzi1}
					jacuzzis={mappedJacuzzis}></CompareList>
				<CompareList
					selectedJacuzzi={jacuzzi2}
					jacuzzis={mappedJacuzzis}></CompareList>
			</Row>
		</Container>
	);
